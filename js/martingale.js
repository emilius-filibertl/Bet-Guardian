  ...
  if (formFilled === true) {
    ...
    if (currentBetPattern === "Player") {
      if (playerWin) {
        // Player Win
        // Update Balance
        martingaleBalance += martingaleBet;
        // Update Bet
        martingaleBet = initialBet;
      ...
      if (bankerWin) {
        ...
        if (halfPayout) {
          martingaleBalance += martingaleBet / 2;
        } else {
          martingaleBalance += martingaleBet;
        }
        // Update Bet
    ...
    balanceOverTime.push(martingaleBalance);
    ...
    document.getElementById("betText").textContent = `${martingaleBet}`;
    // Game Overview
    // Left Container
...
const inputData = function () {
  if (formFilled === false) {
    ...
    // Initialize Value
    martingaleBalance = initialBalance;
    martingaleBet = initialBet;
