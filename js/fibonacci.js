  ...
  if (formFilled === true) {
    ...
    if (currentBetPattern === "Player") {
      if (playerWin) {
        ...
        fibonacciBalance -= fibonacciBet;
        // Update Bet
        if (fibonacciIndex < fibonacciSequence.length - 1) {
          fibonacciIndex += 1;
        }
        fibonacciBet = initialBet * fibonacciSequence[fibonacciIndex];
    ...
    balanceOverTime.push(fibonacciBalance);
    ...
    document.getElementById("betText").textContent = `${fibonacciBet}`;
    // Game Overview
    // Left Container
...
const inputData = function () {
  if (formFilled === false) {
    ...
    // Initialize Value
    fibonacciBalance = initialBalance;
    fibonacciBet = initialBet * fibonacciSequence[fibonacciIndex];
